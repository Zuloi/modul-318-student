...
namespace SwissTransport
{
    public class Transport : ITransport
    {
        ...
        {
            var request = CreateWebRequest("http://transport.opendata.ch/v1/stationboard?station=" + station + "&limit=" + 8);
            var response = request.GetResponse();
            var responseStream = response.GetResponseStream();
        ...
        public Connections GetConnections(string fromStation, string toStation, string date, string time)
        {
            ...
            var request = CreateWebRequest("http://transport.opendata.ch/v1/connections?from=" + fromStation + "&to=" + toStation +"&date=" + date + "&time=" + time + "&limit=" +6);
            var response = request.GetResponse();
            var responseStream = response.GetResponseStream();
